...
async function fetchHttpRequest( url, options ) {
	try {
		...
		if ( error.name === 'TimeoutError' ) {
			// Request timed out.
			error.output = [
				'The request timed out, please try again. If the issue persists, please report to the plugin maintainer.',
			]
			error.ok = false
			console.error( error )
			...
			error.output = [
				'There was a problem communicating with the server, please try again. If the issue persists, please report to the plugin maintainer.',
			]
			error.ok = false
			console.error( error )
		}
